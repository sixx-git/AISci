  ...
  const cls =
    stance === 'support'
      ...
      : stance === 'refute'
        ...
        : 'bg-gray-500/15 text-bp-muted border-gray-500/30';
  return (
    ...
    <div>
      ...
      {items.length === 0 ? (
        ...
        <div className="space-y-3">
          {items.map((ev) => (
            <div key={ev.evidence_id} className="p-3 rounded-lg border border-bp-border bg-gray-850">
...
export function EvidenceChainDrawer({
  ...
  return (
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="relative ml-auto w-full max-w-2xl h-full bg-bp-base border-l border-bp-border shadow-2xl overflow-y-auto animate-slide-in">
        <button
          onClick={onClose}
          className="absolute top-4 right-4 p-2 rounded-lg hover:bg-bp-panel text-bp-muted hover:text-bp-text transition-colors z-10"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <div className="p-6">
          <div className="mb-6 pt-2">
            ...
            <h2 className="text-xl font-bold text-bp-text mb-2">{hypothesisTitle}</h2>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {evidenceChain.revision_history.map((rev, idx) => (
                      <div key={idx} className="p-3 rounded-lg border border-bp-border bg-gray-850 text-xs text-bp-text">
            ...
            <div className="space-y-4">
              {evidenceList.length === 0 ? (
                ...
                evidenceList.map((ev, idx) => (
                  <div key={ev.id || idx} className="p-4 rounded-lg border border-bp-border bg-gray-850">
                    ...
                    {ev.quote_text && (
                      <div className="mb-3 p-3 rounded bg-bp-panel/50 border border-bp-border/50">
