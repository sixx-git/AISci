> **用途**: 网页式文献推荐  
> **输入**: 用户研究问题 + 研究领域（仅此两项）  
> **输出**: 子主题 + 论文清单（含 relevance_score）+ 可选 API 补搜 query
...
5. 每篇论文必须给出整数 `relevance_score`（0–10）：10=直接回答核心；6–9=高度相关方法/数据/场景；3–5=弱相关或仅同领域；0–2=无关。系统将直接用该分数做入库门控，请认真打分
6. 优先给出可检索的 **DOI 或 arXiv ID**；禁止编造不存在的 ID
7. 若无 DOI/arXiv，需给出完整标题、作者、年份
8. 排除：仅因 privacy / learning / survey 等泛词沾边、但场景与方法明显不符的论文（如泛 XAI 综述、泛医学 CV 综述，除非原题明确涉及）
9. 可包含中文期刊/会议论文（可能无 DOI）
10. 输出 1–3 条英文 `search_queries`，供系统在推荐不足时做 API 补搜（空格分隔关键词，不用布尔符）
