...
def text_box(slide, left, top, width, height, text, size=18, color=DARK_TEXT,
             bold=False, align=PP_ALIGN.LEFT, line_spacing=1.2):
    tb = slide.shapes.add_textbox(Inches(left), Inches(top), Inches(width), Inches(height))
    tf = tb.text_frame; tf.word_wrap = True
    p = tf.paragraphs[0]; p.alignment = align
    r = p.add_run(); r.text = text
    r.font.size = Pt(size); r.font.color.rgb = color; r.font.bold = bold; set_font(r)
    p.line_spacing = line_spacing
    return tb

def bullets(slide, left, top, width, height, items, size=14, color=DARK_TEXT, gap=6):
    tb = slide.shapes.add_textbox(Inches(left), Inches(top), Inches(width), Inches(height))
    tf = tb.text_frame; tf.word_wrap = True
    for i, it in enumerate(items):
        ...
        if head:
            r = p.add_run(); r.text = "● " + head; r.font.size = Pt(size); r.font.color.rgb = NAVY; r.font.bold = True; set_font(r)
            r2 = p.add_run(); r2.text = "　" + body; r2.font.size = Pt(size); r2.font.color.rgb = color; set_font(r2)
        else:
            r = p.add_run(); r.text = "● " + body; r.font.size = Pt(size); r.font.color.rgb = color; set_font(r)
    return tb
...
def sub_header(slide, text, left=3.3, top=0.3, width=9.6, size=14):
    text_box(slide, left, top, width, 0.6, "——" + text, size=size, color=SUB_TEXT)

def page_num(slide, n, total):
    text_box(slide, 12.1, 6.95, 1.0, 0.4, f"{n}/{total}", size=11, color=SUB_TEXT, align=PP_ALIGN.RIGHT)

def content_slide(badge_text, subtitle, n, total):
    s = prs.slides.add_slide(BLANK)
    add_bg(s, COB); badge(s, badge_text); sub_header(s, subtitle); gold_line(s, 0.35, 1.0, 12.6, 2)
    return s
...
def card(slide, left, top, width, height, title, desc, action=None, tsize=12.5, dsize=10.5):
    ...
    sh.line.color.rgb = RGBColor(0xD8, 0xDE, 0xEA); sh.line.width = Pt(1)
    ...
    r = p.add_run(); r.text = title; r.font.size = Pt(tsize); r.font.bold = True; r.font.color.rgb = NAVY; set_font(r)
    p2 = tf.add_paragraph(); p2.space_after = Pt(2 if action else 0); p2.line_spacing = 1.05
    r2 = p2.add_run(); r2.text = desc; r2.font.size = Pt(dsize); r2.font.color.rgb = DARK_TEXT; set_font(r2)
    if action:
        p3 = tf.add_paragraph(); p3.line_spacing = 1.05
        r3 = p3.add_run(); r3.text = "对策：" + action; r3.font.size = Pt(dsize); r3.font.color.rgb = GREEN; set_font(r3)
    return sh

def stat_card(slide, left, top, width, height, big, label):
    sh = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(left), Inches(top), Inches(width), Inches(height))
    sh.fill.solid(); sh.fill.fore_color.rgb = RGBColor(0xEC, 0xF1, 0xF9); sh.line.color.rgb = ACCENT; sh.line.width = Pt(1)
    tf = sh.text_frame; tf.word_wrap = True; tf.vertical_anchor = MSO_ANCHOR.MIDDLE
    tf.margin_left = Inches(0.1); tf.margin_right = Inches(0.1)
    p = tf.paragraphs[0]; p.alignment = PP_ALIGN.CENTER
    r = p.add_run(); r.text = big; r.font.size = Pt(30); r.font.bold = True; r.font.color.rgb = NAVY; set_font(r)
    p2 = tf.add_paragraph(); p2.alignment = PP_ALIGN.CENTER
    r2 = p2.add_run(); r2.text = label; r2.font.size = Pt(12); r2.font.color.rgb = SUB_TEXT; set_font(r2)
    return sh

TOTAL = 7

# ============ 1. COVER ============
s = prs.slides.add_slide(BLANK); add_bg(s, CB)
text_box(s, 1.0, 2.0, 11.3, 1.2, "联邦智研 AISci", size=54, color=WHITE, bold=True, align=PP_ALIGN.CENTER)
gold_line(s, 5.2, 3.25, 2.9, 3)
text_box(s, 1.0, 3.5, 11.3, 0.7, "面向《Science》125 前沿科学问题的多智能体科研自动化系统", size=20, color=LIGHT_GOLD, align=PP_ALIGN.CENTER)
text_box(s, 1.0, 4.45, 11.3, 0.6, "赛题指标达成与阶段进展报告", size=22, color=WHITE, bold=True, align=PP_ALIGN.CENTER)
...
s = content_slide("组队与指标", "团队构成 + 核心指标高达成", 1, TOTAL)
text_box(s, 0.5, 1.12, 5.6, 0.36, "（一）组队情况", size=14, color=NAVY, bold=True)
card(s, 0.5, 1.5, 5.6, 1.5, "1.1 团队构成", "总计 8 人（本科生 2 + 研究生 6），学科交叉互补，覆盖计算机/AI/数据科学，支撑多智能体与科研自动化全栈开发。", tsize=11, dsize=8.5)
card(s, 0.5, 3.1, 5.6, 1.7, "1.2 主答辩人员", "梁亮雨（负责人，大三），获数学竞赛省二、交科赛国三等；当前短板：暂无答辩经验（已列入卡点对症施策）。", tsize=11, dsize=8.5)
card(s, 0.5, 4.9, 5.6, 1.5, "1.3 开发投入", "核心开发人员 7.13–8.1 留校集中开发，已完成大部分工作，为 8 月中旬交付奠定坚实基础。", tsize=11, dsize=8.5)
text_box(s, 6.5, 1.12, 6.3, 0.36, "（二）指标达成（总体完成率）", size=14, color=NAVY, bold=True)
stat_card(s, 6.5, 1.5, 2.9, 1.15, "8/9", "评分指标已达成")
stat_card(s, 9.6, 1.5, 2.9, 1.15, "88.9%", "完全达成率")
stat_card(s, 6.5, 2.78, 2.9, 1.15, "94.4%", "加权达成率(部分0.5)")
stat_card(s, 9.6, 2.78, 2.9, 1.15, "3/3", "赛道主线 A/B/B′ 全达成")
bullets(s, 6.5, 4.15, 6.0, 2.4, [
    "9 项中 8 已达成 / 1 部分达成 / 0 未达成。",
    "A/B/B′ 三项主线任务全部“已达成”。",
    "仅“多模态广度”一项为待补强的部分达成指标。",
], size=11, gap=7)

# ============ 4. (二)b 指标总览大表 ============
s = content_slide("指标总览", "9 项评分指标 + 3 赛道主线达成状态", 2, TOTAL)
ind = [
    ...
    ["技术深度", "多模态数据处理/生成/推理/交互", "10", "一般", "部分达成", "已落地 VLM 图像理解与多模态证据构建；音频/视频/化学式/谱图等模态尚未纳入统一证据链"],
...
for ci, h in enumerate(hdr):
    ...
    r = p.add_run(); r.text = h; r.font.size = Pt(10); r.font.bold = True; r.font.color.rgb = WHITE; set_font(r)
for ri, row in enumerate(ind, start=1):
    for ci, val in enumerate(row):
        ...
        r = p.add_run(); r.text = val; r.font.size = Pt(8.5); r.font.name = FONT
        if ci == 4:
...
s = content_slide("差距与对接", "多模态差距·核心/一般区分 · 单位对接", 3, TOTAL)
...
# 左：差距与区分
text_box(s, 0.5, 1.1, 5.9, 0.34, "（二）差距与区分", size=13, color=NAVY, bold=True)
card(s, 0.5, 1.5, 5.9, 1.55, "2.3 部分达成差距·多模态广度(约6–7分)",
     "已落地 VLM 图像理解+多模态证据构建；音频/视频/化学式/谱图等尚未进证据链；目标≥8 分。", tsize=10.5, dsize=8.5)
text_box(s, 0.5, 3.15, 2.9, 0.32, "【核心指标·全达成】", size=10.5, color=NAVY, bold=True)
...
text_box(s, 3.4, 3.15, 2.9, 0.32, "【一般指标】", size=10.5, color=NAVY, bold=True)
...
# 右：单位对接
text_box(s, 6.7, 1.1, 6.1, 0.34, "（三）单位对接", size=13, color=NAVY, bold=True)
...
text_box(s, 6.7, 5.6, 6.2, 1.1, "小结：呈现“高频答疑 + 直达技术负责人”特点，在路线对齐、难点澄清与资源供给三方面形成有效支撑。", size=11, color=NAVY, bold=True)

# ============ 7. (四) 堵点卡点（技术/资源/协调政策 合并） ============
s = content_slide("堵点卡点", "4.1 技术难题 · 4.2 资源缺口 · 4.3/4.4 协调政策（共 9 项）", 4, TOTAL)
col_defs = [(0.35, "【4.1 技术难题】"), (4.65, "【4.2 资源缺口】"), (8.95, "【4.3/4.4 协调政策】")]
tech = [
...
for ci, (cl, chead) in enumerate(col_defs):
    text_box(s, cl, 1.1, 4.1, 0.36, chead, size=12.5, color=NAVY, bold=True)
    for ri in range(3):
        t, d, a = cols[ci][ri]
        card(s, cl, top0 + ri * (ch + 0.06), 4.05, ch, t, d, action=a, tsize=10, dsize=8)

# ============ 10. (五) 工作计划表 ============
s = content_slide("工作计划", "五项里程碑，8/15 前交付闭环", 5, TOTAL)
plan = [
    ["里程碑任务", "时间节点", "负责人"],
    ["多模态技能层扩展（化学式/谱图/音视频抽取 + 统一证据表示）", "8/05 前", "梁亮雨"],
...
for ri, row in enumerate(plan):
    for ci, val in enumerate(row):
        ...
        r = p.add_run(); r.text = val; r.font.size = Pt(12 if ri else 13)
        r.font.color.rgb = WHITE if ri == 0 else DARK_TEXT; r.font.bold = (ri == 0); set_font(r)

# ============ 8. (六) 综合自评（潜力/优势/风险 合并） ============
s = content_slide("综合自评", "获奖潜力高 · 主要优势 · 最大风险与突围", 6, TOTAL)
text_box(s, 0.5, 1.12, 12.3, 0.6, "6.1 冲刺获奖潜力：☑ 高　理由：核心指标全达成、闭环完整可复现、差异化创新明确；仅一项待补强（多模态广度）且路径明确，8/15 前可收敛。", size=12.5, color=NAVY, bold=True)
text_box(s, 0.5, 1.92, 6.0, 0.4, "6.2 主要优势", size=14, color=NAVY, bold=True)
...
r.font.size = Pt(15); r.font.bold = True; r.font.color.rgb = NAVY; set_font(r)
...
r2.font.size = Pt(12); r2.font.color.rgb = DARK_TEXT; set_font(r2)
p3 = tf.add_paragraph(); p3.space_before = Pt(10)
r3 = p3.add_run(); r3.text = "突围策略：聚焦唯一弱项“多模态广度”8/15 前显著收敛；以“可审计闭环+零幻觉引用+可复现”三张差异化名片精准命中评审，并在答辩现场强化记忆点。"
r3.font.size = Pt(11.5); r3.font.color.rgb = GREEN; r3.font.bold = True; set_font(r3)
