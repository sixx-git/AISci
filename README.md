...
标准 Pipeline 为 7 阶段顺序执行，支持人工审核 Gate（HITL）、综合质量分（CQS）、完整审计链导出等能力。项目模式分为**通用**与**联邦学习**两种，由 `project_mode` 闸口隔离。
...
| 预定义错误模式 | `CoordinatorAgent.STAGE_ERROR_PATTERNS` | 覆盖 6 大阶段共 12+ 条预定义错误规则（严重/高/中/低四级） |
